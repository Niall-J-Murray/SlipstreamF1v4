...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (user) {
      setCurrentUser(user);
      if (isAdmin(user)) {
        tailoredNavLinks = navigation.filter(
          (value) =>
            value.name === "Home" ||
            value.name === "Profile" ||
            ...
            <div className="relative flex h-16 items-center justify-between">
                ...
                <div className="hidden sm:ml-6 sm:block">
                  <div className="flex space-x-4">
                    {tailoredNavLinks
                      .filter(
                        (item) =>
                          item.name !== "Sign Out" && item.name !== "Profile"
                      )
                      .map((item) => (
                        <Link
                          key={item.name}
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                ...
                <Menu as="div" className="relative ml-3">
                  <div>
                    <Menu.Button className="btn btn-dropdown flex rounded-full bg-black-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-black-800 p-1">
